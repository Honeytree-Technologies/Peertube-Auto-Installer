...
validate_port() {
    local port=$1
    local excluded_ports=("80" "443" "9000" "1935")
...
read -p "Enter the DB USER NAME (Default: peertube): " db_user
if [ -z ${db_user} ] ; then
  db_user=peertube
fi
...
while true; do
  ...
  if [ -n "$smtp_host" ]; then
    ...
    echo "SMTP hostname cannot be empty. Please enter SMTP hostname."
  fi
done
...
while true; do
  read -p "Enter SMTP user name: " smtp_user
  if [ -n "$smtp_user" ]; then
    break
  else
...
while true; do
  ...
  if [ -n "$smtp_from" ]; then
    ...
    echo "SMTP_FROM cannot be empty. Please enter smtp from."
  fi
done
...
while true; do
  read -p "Enter a port number (1-65535, excluding 80, 443, 9000 and 1935): " port
  # Validate the input
  validate_port "$port"
  case $? in
    0)
...
if nginx -v &>/dev/null; then
  ...
  rm /etc/nginx/sites-available/peertube
  rm /etc/nginx/sites-enabled/peertube
else
  sudo apt-get update
...
touch /etc/nginx/sites-available/peertube

cat <<nginx_content >>/etc/nginx/sites-available/peertube
server {
...
echo "Congratulations your setup is complete, now can login on  https://${domain_name}"
echo "Use this credentials  Email :- ${admin_email}    and password:-  ${admin_password}"
